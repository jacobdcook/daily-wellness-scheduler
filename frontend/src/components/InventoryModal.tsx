...
import { X, Plus, Minus, AlertTriangle, ShoppingCart, Plane, RefreshCw, DollarSign, Link2 } from "lucide-react";
...
export function InventoryModal({ isOpen, onClose, settings, schedule, onUpdate, onOpenTravelPlanner }: InventoryModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            // Re-populate inventory from schedule when modal opens
            const inventory = { ...(settings.inventory || {}) };
    ...
    }, [isOpen, schedule, settings.inventory]);
    ...
    return (
        <div className="fixed inset-0 z-[10000] overflow-hidden flex items-center justify-center">
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col mx-4 border border-gray-200 dark:border-gray-800">
                ...
                <div className="p-4 overflow-y-auto flex-grow space-y-4">
                    {inventoryItems.length === 0 ? (
                        <div className="text-center text-gray-500 py-8">
                            {schedule && Object.values(schedule).flat().length > 0 ? (
                                <div>
                                    <p className="mb-2">Loading items from schedule...</p>
                                    <p className="text-xs text-gray-400">If items don't appear, try refreshing the page.</p>
                                </div>
                            ) : (
                                <div>
                                ...
                                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            ...
                                            className="w-8 h-8 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full text-gray-500 dark:text-gray-300 transition-colors"
                                        >
                                            <Minus size={16} />
                                        </button>
                                        <input
                                            ...
                                            className="w-16 text-center font-mono font-bold text-lg border rounded py-1 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none bg-white dark:bg-gray-900 dark:border-gray-700 dark:text-white"
                                        />
                                        <button
                                            onClick={() => updateStock(name, 1)}
                                            className="w-8 h-8 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full text-gray-500 dark:text-gray-300 transition-colors"
                                    ...
                                    <label className="flex flex-col gap-1 text-gray-600 dark:text-gray-300">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button
                                                ...
                                                className="px-3 py-2 text-xs border rounded-md hover:bg-gray-100 dark:hover:bg-gray-800"
                                            >
                                                Smart
                                            </button>
                                        </div>
                                ...
                                <div className="grid sm:grid-cols-2 gap-3 text-sm">
                                    ...
                                    <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                                        ...
                                        {item.auto_refill && reorderDate && (
                                            <span className="text-[11px] text-gray-500 dark:text-gray-400">Next target {reorderDate}</span>
                                        )}
                                    </label>
                                </div>
                ...
                <div className="p-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 flex justify-end gap-3 rounded-b-xl">
                    <button onClick={onClose} className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800/80">Cancel</button>
